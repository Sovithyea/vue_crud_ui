    ...
    <v-app>
        ...
        <!-- <v-navigation-drawer v-model="drawer" dark app> -->
            <v-list shaped>
                ...
                <v-list-item dark :to="{ name: 'post.index' }">
                    <v-list-item-icon>
                    <v-icon color="info">mdi-clipboard-outline</v-icon>
                    </v-list-item-icon>
                    <v-list-item-content>
        ...
        <v-footer padless>
            <v-card flat tile width="100%" dark class="text-center">
                <v-card-text class="white--text">{{ new Date().getFullYear() }} —<strong>powered_by Tiv Sovithyea</strong></v-card-text>
            </v-card>
        </v-footer>
      </v-app>
    </template>
